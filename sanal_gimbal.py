#Redis üzerinden bbox okunur, sanal gimbal döümüşümleri uygulanır ve raw ile sanal koordinatlar terminale yazdırılır.
import numpy as np
import math
import json
import time
...
class SanalGimbal:
    def __init__(self):
        ...
        print("[SanalGimbal] Redis bağlantısı kuruldu, 'tracker_bbox' kanalı dinleniyor.")
        ...
        # MAVLink (sadece attitude okumak için)
        self.master = mavutil.mavlink_connection(f'udpin:127.0.0.1:{UAV_PORT}')
        self.master.wait_heartbeat()
        print(f"[SanalGimbal] MAVLink bağlantısı kuruldu (sys={self.master.target_system}).")

        # MAVLink reader thread — cache'i güncel tutar
    ...
    def _apply_virtual_gimbal(self, bbox):
        ...
        # Attitude oku (cache'ten, non-blocking)
    ...
    def _redis_listener(self):
        """Redis pub/sub dinler, parse edip latest_bbox'a yazar."""
    ...
    # ─── Processor ───
    def _processor(self):
        """30 Hz'de en güncel bbox'ı okuyup sanal gimbal dönüşümünü uygular."""
        period = 1.0 / 30.0
