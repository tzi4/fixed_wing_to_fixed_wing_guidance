...
DETECTION_CHANNEL = "tracker_bbox"  # The consumer subscribes with this
...
def publish_detection(msg: dict):
    """send the JSON format that the consumer expects to the channel"""
    try:
...
while True:
    try:
        ...
        if n > 0:
            ...
            # PUB/SUB publication — CONSUMER CRITICAL
            publish_detection({
                "source": "yolo",
                "bbox": [x1, y1, x2, y2],     # xyxy (consumer already converts this to xywh)
                "conf": conf,
                "detected": True,
                "ts": ts_now,
                "img_w": int(w),
            ...
            # PUB/SUB release — YOLO refreshes watch, consumer understands 'no-detection'
            publish_detection({
                "source": "yolo",
                "detected": False,
                "ts": ts_now,
