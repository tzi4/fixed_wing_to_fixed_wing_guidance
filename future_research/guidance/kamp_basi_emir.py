...
class MavlinkManager(threading.Thread):
    ...
    def send_altitude_target(self, altitude_m, alt_rate=0.8):
        with self.lock:
            try:
                # COMMAND_LONG -> COMMAND_INT dönüşümünde frame varsayılanı
                # MAV_FRAME_GLOBAL_RELATIVE_ALT'tır (GCS_Common.cpp).
                # ArduPlane tırmanma/alçalma hızını param3'ten okur
                # (GCS_MAVLink_Plane.cpp, handle_command_int_guided_slew_commands);
                # param3=0 gönderilirse hız sınırsız (1000 m/s) kabul edilir.
                self.master.mav.command_long_send(
                    self.master.target_system,
                    self.master.target_component,
    ...
    def send_speed_target(self, airspeed_ms, speed_type=0):
        ...
        # param3: throttle (0 -> mevcut throttle'a dokunma; airspeed kabul edilirse zaten kullanılmaz)
        # ArduPlane handler'ı (commands_logic.cpp do_change_speed) airspeed'i yalnızca
        # AIRSPEED_MIN <= hız <= AIRSPEED_MAX aralığında kabul eder; aksi halde komut reddedilir.
        with self.lock:
            try:
                self.master.mav.command_long_send(
                    self.master.target_system,
...
class AutopilotController:
    def __init__(self, connection_str='udp:127.0.0.1:14552'):
        ...
        self.Kp_rate = 1.2      # 1° açısal hata → 5 deg/s dönüş hızı
        self.Ki_rate = 0.0
        self.Kd_rate = 0.12     # Hızlı hareket eden hedefe tepki için
        ...
        # --- ALTITUDE RATE PID (tırmanma/alçalma hızı kontrolü) ---
        self.Kp_alt_rate = 1.5      # 1° dikey hata → 0.5 m/s irtifa değişim hızı
        self.Ki_alt_rate = 0.0
        self.Kd_alt_rate = 0.1      # Hızlı dikey harekete tepki için

        self.integral_error_alt_rate = 0.0
        self.prev_error_alt_rate = 0.0
        self.prev_derivative_alt_rate = 0.0

        self.min_alt_rate = 0.3     # Minimum tırmanma/alçalma hızı (m/s)
        self.max_alt_rate = 5.0     # Maksimum tırmanma/alçalma hızı (m/s)

        # --- SPEED (HIZ) PID ---
        # KAVRAM: coverage (hedefin ekrandaki boyutu) ~ 1/mesafe => bir MENZİL sinyali.
        # Hız ise menzilin TÜREVİNİ (kapanma hızını) belirler. Boyut, hızın integralidir.
        # Sabit bir takip mesafesi tutmak = hedefin hızına KİLİTLENMEK demektir.
        #   coverage = max(yatay_kaplama%, dikey_kaplama%)
        # Hedef setpoint'ten küçükse/uzaksa (coverage düşük) -> error pozitif -> hızlan.
        # Hedef setpoint'ten büyükse/yakınsa (coverage yüksek) -> error negatif -> yavaşla.
        #
        # ÖNEMLİ: target_coverage_pct artık "kapanmak istediğim boyut" değil,
        # "korumak istediğim takip mesafesi" (kilit eşiğine yakın). I terimi steady-state'i
        # hedefin hızına oturtur, böylece base_speed'in tam değeri kritik olmaktan çıkar.
        self.Kp_speed = 0.2      # 1% kaplama hatası -> 0.2 m/s hız değişimi (yumuşak)
        self.Ki_speed = 0.03     # KÜÇÜK: çıkışı yavaşça hedef hıza oturtur (yalnız setpoint civarı)
        self.Kd_speed = 0.0      # OPSİYONEL sönüm (filtrelenmiş coverage-rate üzerine); önce 0 dene

        self.integral_error_speed = 0.0
        self.prev_error_speed = 0.0
        self.prev_derivative_speed = 0.0

        self.min_speed = 14.0    # Minimum hava hızı (m/s) - AIRSPEED_MIN(13) biraz üstü
        self.max_speed = 22.0    # Maksimum hava hızı (m/s) - takipte 25'e fırlamasın
        self.base_speed = 17.0   # Nötr hız = AIRSPEED_CRUISE (I terimi offset'i halleder)

        self.lock_threshold_pct = 5.0      # "yatay VEYA dikey min %5" kilit/geçerlilik eşiği
        self.target_coverage_pct = 6.0     # KORUNMAK İSTENEN takip mesafesi (kilit eşiğine yakın)
        self.speed_deadzone_pct = 0.0      # KAPALI: hız bir rate komutu, deadzone steady-state kaymasına yol açar (EMA+slew gürültüyü zaten bastırır)
        self.speed_integral_band = 5.0     # I yalnız bu bandın İÇİNDE biriksin (uzakta windup yok)
        self.speed_slew_rate = 1.0         # cmd_speed değişim hız limiti (m/s / s)
        self.coverage_alpha = 0.35         # coverage EMA katsayısı (~4-5 örnek); düşük = daha yumuşak
        self.filtered_coverage = self.target_coverage_pct  # EMA durumu (setpoint'ten başlat)
    ...
    def coverage_metric(self, bbox_w, bbox_h):
        # "yatayda VEYA dikeyde" kriterine uygun max-eksen kaplama yüzdesi
        h_cov = (bbox_w / self.camera_width) * 100.0    # yatay kaplama %
        v_cov = (bbox_h / self.camera_height) * 100.0   # dikey kaplama %
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # --- ALTITUDE RATE PID (tırmanma/alçalma hızı kontrolü) ---
                alt_rate_error = abs(p_y)  # Dikey açısal hatanın büyüklüğü (deadzone uygulanmış)
                ...
                # --- 3. SPEED (HIZ) KONTROLÜ ---
                # coverage ~ 1/mesafe (MENZİL); hız menzilin türevini belirler.
                # Sabit takip mesafesi tutmak = hedef hızına kilitlenmek.

                # (a) Coverage'ı EMA ile filtrele (bbox titreşimini bastır)
                ...
                # (b) Hata = takip mesafesi setpoint'i - filtrelenmiş kaplama
                speed_error = self.target_coverage_pct - self.filtered_coverage

                # (c) Deadzone: setpoint'e yakınken hızı oynatma (TECS'i boşuna kıpırdatma)
                ...
                # (d) Türev (filtrelenmiş hata üzerine, ekstra low-pass)
                ...
                # (e) İntegral: YALNIZ setpoint civarında biriksin (uzakta windup olmasın).
                #     Böylece I, gross yaklaşmayı değil hedef hızına ince oturmayı yapar.
                if gorev == "Goruntulu" and self.mavlink.current_mode == "GUIDED":
                ...
                # (f) PID çıkışı (base_speed = cruise etrafında)
                ...
                # (g) Slew-rate limiti: TECS adım görüp pitch'i çalmasın (irtifa kaybının kökü buydu)
                ...
                if current_time - self.last_cmd_send_time >= 0.1:
                    if gorev == "Goruntulu" and self.mavlink.current_mode == "GUIDED":
                        ...
                        command_sent = 1
                        print(f"[{gorev}] OTONOM: HedefYön: {target_heading:.1f}° | HRate: {heading_rate:.1f}°/s | İrtifa: {target_alt:.1f}m | ARate: {altitude_rate:.2f}m/s | Hız: {cmd_speed:.1f}m/s | Kaplama: {self.filtered_coverage:.1f}% (hedef %{self.target_coverage_pct:.0f})")
                    else:
