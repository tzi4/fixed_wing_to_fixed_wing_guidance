...
# 3.1 PRO UAV GUIDANCE IMPLEMENTATION
#Combine the TZI architecture, including modular threads, angular errors
#and failsafe behavior, with Emir's rate-based PID flight dynamics,
#coverage-based speed controller and exponential moving-average filter.
...
RESOLUTION_W = 1280
RESOLUTION_H = 720
CAMERA_FOCAL_LENGTH = 3045.737  # Emir's goat_gimbal.py fx value
MIN_ALTITUDE = 10.0
MAX_ALTITUDE = 200.0
MAX_DELTA_HEADING = 35.0  # Limit from Emir
MAX_DELTA_ALTITUDE = 10.0 # Limit from Emir
UAV_PORT = '14552' # Emir's code uses 14552, tzi's code uses 14553.

# Camera->Body rotation (accepting camera z forward, x right, y down)
...
class FusionGuidance_3_1_Pro(System_3_1_Pro):
    def __init__(self):
        ...
        # PID parameters adapted from Emir's code
        self.Kp_heading = 3.0
        self.Ki_heading = 0.0
        self.Kd_heading = 0.0
        self.Kp_rate = 1.2
        ...
        self.cmd_thread = TestCommander_3_1_Pro(self.mavlink_handler, rate_hz=10) # 10Hz command refresh
    ...
    def guide_aircraft(self, bbox, current_time):
        ...
        if self.K_inv is not None:
            ...
            # Angular errors in degrees, following the TZI method
            ...
            # Derivative: low-pass filter (EMA, following Emir's method)
            alpha = 0.05
            ...
            # --- 1. HEADING & HEADING RATE CONTROL (3.1 PRO) ---
            ...
            # --- 2. ALTITUDE AND ALTITUDE-RATE CONTROL (3.1 PRO) ---
            ...
            # --- 3. AIRSPEED CONTROL (Coverage Based) ---
    ...
    def _redis_listener(self):
        self.logger.debug("Redis listener thread started (3.1 Pro).")
    ...
    def _vision_processor(self):
        self.logger.debug("Vision processor thread started (3.1 Pro).")
        period = 1.0 / 30.0
            ...
            if bbox is not None and bbox_time != last_processed_time:
                ...
                if last_target_time > 0.0:
                    ...
                    if time_since_last > 5.0:
                        ...
                        self.logger.warning("[FAILSAFE - 3.1 PRO] TARGET HAS BEEN MISSING FOR MORE THAN 5 SECONDS!")
    ...
    def run(self):
        ...
        self.logger.debug("All threads started (3.1 Pro). Main thread is waiting...")
