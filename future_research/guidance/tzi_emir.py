...
# Simülasyon kamerası:
# ~/catkin_ws/src/iq_sim/models/emir-gazebo-plane/model.sdf
# 1280x720, horizontal_fov=0.27 rad -> fx=fy=4711.9057 px.
CAMERA_HFOV_RAD = 0.27
CAMERA_FX = (RESOLUTION_W / 2.0) / math.tan(CAMERA_HFOV_RAD / 2.0)
CAMERA_FY = CAMERA_FX            # Kare piksel varsayımı

# GERÇEK KAMERA PROFİLİ — goat_gimbal.py ile havada kullanılan değerler.
# Uçuştan önce aşağıdaki üç satırı yorumdan çıkar; bunlar üstteki simülasyon
# değerlerini ezer. Çözünürlük her iki profilde de 1280x720.
...
class TargetLockTracker:
    """Yarışma geometrisini kesintisiz süre şartıyla izler."""
...
class TestCommander(threading.Thread):
    ...
    def _send_airspeed(self, airspeed_ms):
        """ArduPlane GUIDED'a ait hız komutunu gönderir."""
        ...
        with self.mavlink_io_lock:
            self.master.mav.command_long_send(
                ...
                0,              # confirmation
                0,              # param1: ArduPlane yalnız airspeed destekliyor
                airspeed_ms,    # param2: hedef airspeed (m/s)
                1.0,            # param3: max ivme (m/s²)
                0, 0, 0, 0
            )
...
class tziGuidance(System):
    ...
    def __init__(self):
        ...
        self.min_speed = 10.0         # SITL AIRSPEED_MIN=9 için emniyet paylı
        self.max_speed = 22.0
        self.base_speed = 18.0        # Hedef uçağın nominal hızı
        self.target_coverage_pct = 6.0    # Korunmak istenen takip mesafesi (%)
        self.speed_integral_band = 7.0    # I sadece bu band içinde biriksin
        self.speed_slew_rate = 2.0        # Max hız değişimi (m/s/s)
        ...
        # Resmî kilit koşulu: hedef alan içinde ve yeterli boyutta 4 s.
    ...
    def _update_lock_state(self, bbox, now, active):
        ...
        if progress_second != self._last_lock_log_second and not acquired:
            ...
            if self.lock_tracker.progress_s > 0.0:
                self.logger.info(
                    f"Kilit ilerlemesi: {self.lock_tracker.progress_s:.1f}/"
                    f"{self.lock_tracker.required_s:.1f} s")
        if acquired and not was_acquired:
            self.logger.warning("🔒 KİLİT ATILDI: resmî koşul 4 saniye kesintisiz sağlandı.")
        return acquired
    ...
    def guide_aircraft(self, bbox, current_time):
        ...
        # Kilit alanı ham kamera karesinde tanımlıdır. Sanal gimbal hatası
        # uçağın pitch/roll hareketini iptal ettiğinden hedefi ataletsel olarak
        # tutar, fakat ekranda kilit alanı dışına sürükleyebilir. Kontrol bu
        # nedenle ham optik hatayı kullanır; stabilize değer yalnız teşhistir.
        ...
        # Integral (band-limited + koşullu anti-windup)
    ...
    def _redis_listener(self):
        ...
        try:
            ...
            if self._running.is_set():
                self.logger.exception("Redis listener hatası")
