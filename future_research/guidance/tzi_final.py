...
RESOLUTION_W = 1280
RESOLUTION_H = 720
CAMERA_FOCAL_LENGTH = 4711.91  # 467.7 * (1280/640) — aynı FOV, ölçeklenmiş ya da 4711.91 mı?
MIN_ALTITUDE = 10
MAX_ALTITUDE = 200
...
class TestCommander(threading.Thread):
    ...
    def _send_airspeed(self, airspeed_ms):
        """MAV_CMD_DO_CHANGE_SPEED (178) — Standart MAVLink hız komutu.
        Sadece GUIDED modundayken airspeed komutuna izin ver.
        param1: SPEED_TYPE (0 = airspeed, 1 = groundspeed)
        param2: hedef hız (m/s)  (-1 = değişiklik yok, -2 = varsayılan)
        param3: throttle (%)     (-1 = değişiklik yok, -2 = varsayılan)
        """
...
class System():
    def __init__(self):
        ...
        self.fx = f_oc  # Yatay focal length (piksel)
        self.fy = f_oc  # Dikey focal length (piksel) — Gazebo kare piksel kullanır
        self.K = np.array([
...
class tziGuidance(System):
    def __init__(self):
        ...
        # ─── AÇISAL DÖNÜŞÜM: KAMERA-BAĞIMSIZ PID/PD ───
        # Tüm hatalar derece cinsinden hesaplanır: atan(pixel_error / fx)
        # Kazançlar tzi.py'deki (640×480, f=467.7) tune değerlerinden türetildi:
        #   Dönüşüm faktörü = f_ref × π/180 = 467.7 × 0.01745 = 8.163
        #   Kp_angular = Kp_pixel × 8.163
        #   Kd_angular = Kd_pixel × 8.163
        # Bu sayede çözünürlük veya focal length değiştiğinde yeniden tune gerekmez.

        # AIRSPEED PID — açısal boyut (derece) cinsinden
        # Orantılı ölçekleme: throttle range [50,127]=77 → airspeed range [10,22.8]=12.8 (scale=0.1662)
        # Orijinal angular K değerleri: Kp=38.37, Ki=3.67, Kd=4.90
        ...
        # Hedef: 52m-73m aralığının ortası (~62.5m)
        self.airspeed_target_angular_deg = 0.505   # (0.590+0.420)/2 = ortanokta
        self.airspeed_deadzone_deg = 0.085         # (0.590-0.420)/2 = yarıçap
        self.airspeed_integral_min = -1.66  # -10.0 × 0.1662
        self.airspeed_integral_max = 9.14   # 55.0 × 0.1662
        self.airspeed_base = 15.0  # Baslangic airspeed degeri (m/s)
        self.airspeed_min = 10.0
        self.airspeed_max = 22.8
        self.airspeed_max_accel = 2.0  # Saniyede en fazla 2.0 m/s hızlan/yavaşla (Slew Rate)
        ...
        # Yumuşak Ölü Bölge — açısal (derece), kamera bağımsız
        self.deadzone_deg = 0.78  # gimballi_roll_pitch.py ile aynı

        # HEADING PD — açısal (derece) cinsinden
        # tzi.py: Kp = 10/320 = 0.03125 deg/px, Kd = 0.02 deg·s/px
        # Dönüşüm: ×8.163
        self.heading_kp = 0.255   # 0.03125 × 8.163
        self.heading_kd = 0.163   # 0.02   × 8.163
        self.heading_prev_error = 0.0

        # ALTITUDE PD — açısal (derece) cinsinden
        # tzi.py: Kp = 5/240 = 0.02083 m/px, Kd = 0.01 m·s/px
        # Dönüşüm: ×8.163
        ...
        # NOT: update() burada çağrılmaz — ilk bbox gelene kadar commander pasif kalır
        # Bu sayede bbox gelmeden uçak 50m'ye tırmanmaz
    ...
    def guide_aircraft(self, bbox, current_time): #bbox: x1,y1,w,h
        ...
        if self.K_inv is not None:
            ...
            # BBox alan hesabı — açısal boyuta dönüştür (kamera bağımsız)
            ...
            # Kapsama yüzdeleri (Yatay ve Toplam Alan)
            ...
            if is_first_pid:
                ...
                # GUIDED moda ilk girişte anlık hızı (airspeed) başlangıç kabul et
            ...
            # Error hesabi: açısal boyut (derece) — kamera bağımsız
            spd_error = self.airspeed_target_angular_deg - bbox_angular_deg

            # Proportional — yumuşak deadzone
            ...
            # --- HEADING PD Kontrolcusu (açısal, derece) ---
            ...
            # Yumuşak Deadzone (derece cinsinden)
            ...
            # --- ALTITUDE PD Kontrolcusu (açısal, derece) ---
            # Piksel hatasını açısal hataya dönüştür — yukarı pozitif
            ...
            # Altitude delta clamp (daha sakin bir tepki için daraltıldı)
    ...
    def _vision_processor(self):
        ...
        Hedef kaybolduğunda 5 saniye coasting, sonra failsafe uygular."""
            ...
            if bbox is not None and bbox_time != last_processed_time:
                ...
                if last_target_time > 0.0:
                    ...
                    if time_since_last <= 5.0:
                        # ── COASTING (5s): TestCommander son heading/alt/throttle değerlerini
                        # göndermeye devam eder — ek işlem gerekmez ──
                        pass
                    else:
                        # ── FAILSAFE: 5 saniye aşıldı! ──
                        # PID/PD state'lerini sıfırla
                        self.airspeed_integral = 0.0
                        self.airspeed_prev_error = 0.0
                        self.heading_prev_error = 0.0
                        self.altitude_prev_error = 0.0
                        ...
                        # Commander'ı deaktive et (uçak otopilota teslim)
                        self.cmd_thread.active = False

                        # Saniyede bir terminale uyarı bas
