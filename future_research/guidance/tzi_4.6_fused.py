...
RESOLUTION_W = 1280
RESOLUTION_H = 720
CAMERA_HFOV_RAD = 0.415  # HFOV from Emir's code (radians)
MIN_ALTITUDE = 10
MAX_ALTITUDE = 200
UAV_PORT = '14553'
...
# ─── DETAILED FLIGHT LOgger (FlightLogger) ───
class FlightLogger:
    def __init__(self, file_prefix="flight_log_fused"):
...
# ─── AUTOPILOT COMMANDER THREAD (TestCommander) ───
class TestCommander(threading.Thread):
    """
    Heading: variable-rate guidance with MAV_CMD_GUIDED_CHANGE_HEADING (43002).
Altitude: variable climb/descent rate with MAV_CMD_GUIDED_CHANGE_ALTITUDE (43001).
Airspeed: acceleration control with MAV_CMD_DO_CHANGE_SPEED (178).
    """
    def __init__(self, mav_handler, rate_hz=5):
        ...
        # Here is the VFR_HUD message (for airspeed measurement)
    ...
    def _send_heading(self, heading_deg, heading_rate):
        """
        param1: 1 = raw magnetic heading (nose direction) param2: heading (degrees, 0-360) param3: heading rate (deg/s) - variable rate
        """
        self.master.mav.command_long_send(
            ...
            heading_rate,  # param3: rotation speed (deg/s)
    ...
    def _send_altitude(self, alt_m, altitude_rate):
        ...
        self.master.mav.command_long_send(
            ...
            altitude_rate,  # param2: climb/descend speed (m/s)
            0, 0, 0, 0,  # param3-6
            alt_m   # param7: target altitude
        )

    def _send_airspeed(self, airspeed_ms):
        """
        param1: SPEED_TYPE (0 = airspeed) param2: target speed (m/s) param3: throttle (-1 = no change)
        """
        ...
        self.master.mav.command_long_send(
            ...
            0,              # confirmation
            0,              # param1: 0 = airspeed
            airspeed_ms,    # param2: target speed (m/s)
            -1,             # param3: throttle
            0, 0, 0, 0      # param4-7
        )

    def _read_telemetry(self):
        """Pymavlink reads telemetry data from its internal cache (non-blocking)."""
    ...
    def run(self):
        ...
        while self.running:
            if self.active:
                ...
                # No hardcoded override prior to flight testing
...
class System():
    def __init__(self):
        ...
        # VISUAL FIELD AND CAMERA CALIBRATION MATRIX
        self.W = RESOLUTION_W
        self.H = RESOLUTION_H
        self.center_x = self.W / 2.0
        self.center_y = self.H / 2.0

        # Focal Length Calculation (via HFOV)
        ...
        # MAVLink data reader thread (cache update)
    ...
    def init_logger(self):
        self.logger = logging.Logger('tzi_fused')
        self.logger.setLevel(logging.DEBUG)
        
        c_handler = logging.StreamHandler()
        log_file_path = 'tzi_fused_guidance.log'
        old_logs_dir = "Logs"
        ...
        if os.path.exists(log_file_path):
            ...
            new_log_file_name = f"tzi_fused_guidance_{timestamp}.log"
...
# ─── UNIFIED GUIDANCE CLASS (tziGuidance) ───
class tziGuidance(System):
    def __init__(self):
        ...
        # ─── CONTROL PARAMS & GAINS (Values ​​Tested by Emir in Flight) ───
        self.Kp_heading = 3.0
        self.Ki_heading = 0.0
        self.Kd_heading = 0.0
        ...
        # HEADING RATE PID (variable rotation speed)
        self.Kp_rate = 1.2
        self.Ki_rate = 0.0
        self.Kd_rate = 0.12
        ...
        # ALTITUDE RATE PID (variable climb/descent rate)
        ...
        # SPEED PID (range Follow Control)
        self.Kp_speed = 0.2
        self.Ki_speed = 0.03
        self.Kd_speed = 0.0
        self.min_speed = 14.0
    ...
    def guide_aircraft(self, bbox, current_time):
        ...
        # 1) Virtual Gimbal Pixel Stabilization (Back-Projection & Re-Projection)
        ...
        # 2) Resolution Independent Angular Error Calculation
        ...
        # Reset PID states if we are not in GUIDED mode
        if current_mode != 'GUIDED':
            ...
            # CSV Write Log (Unguided)
            self._log_state(
                current_time, 0.033, task, "INACTIVE", 1, 0, 0, 0.0,
        ...
        # 3) Low-Pass Filter Derivative Calculation
        alpha = 0.05
        ...
        # 4) Integral Calculus and Deadzone
        ...
        # If the target is below (p_y positive), altitude MUST BE REDUCED
        ...
        # Variable Climb/Descent Rate (Variable Altitude Rate)
        ...
        # 7) SPEED CONTROL (Via BBox Coverage %)
        ...
        # Velocity integral accumulates only in the setpoint band
        if abs(speed_error) < self.speed_integral_band:
            ...
            self.integral_error_speed *= 0.95  # my slow end
        ...
        # Slew Rate Acceleration Limiter (1.0 m/s/s)
        ...
        # Update variables and send to Commander
        ...
        # Check if Commander is sending data
        command_sent = 1 if self.cmd_thread.active else 0
        data_age_ms = (current_time - current_time) * 1000.0  # 0 because it is processed instantly

        # Save to Detailed CSV File
        self._log_state(
            current_time, dt, task, "TRACKING", 1, command_sent, 0, data_age_ms,
            bbox_x, bbox_y, bbox_w, bbox_h, obj_x, obj_y, target_area,
            stab_x, stab_y, stab_x - obj_x, stab_y - obj_y,
    ...
    def _log_state(self, current_time, dt, task, system_state, target_found,
        ...
        row = [
            ...
            self.cmd_thread.last_mode if self.cmd_thread.last_mode else "UNKNOWN", task, system_state, target_found,
    ...
    def _vision_processor(self):
        ...
        while True:
            ...
            if bbox is not None and bbox_time != last_processed_time:
                ...
                # TARGET LOST: Coasting or Failsafe
                if last_target_time > 0.0:
                    time_since_last = current_time - last_target_time
                    
                    if time_since_last <= 5.0:
                        # ── COASTING (5s) ── TestCommander continues sending last targets values
                        pass
                    else:
                        # ── FAILSAFE (5s Hanged) ── PID Reset Integral and Prev states
                        self.integral_error_x = 0.0
                        self.integral_error_y = 0.0
                        self.integral_error_rate = 0.0
                        self.integral_error_alt_rate = 0.0
                        ...
                        # Get updated location_data/yon from autopilot
                        att_msg = self.mavlink_handler.master.messages.get('ATTITUDE', None)
                        loc_msg = self.mavlink_handler.master.messages.get('GLOBAL_POSITION_INT', None)
                        ...
                        # CSV Write Log (In Failsafe state)
                        task_bytes = self.r.get('task')
                        task = task_bytes.decode('utf-8') if task_bytes else "Unknown"
                        
                        self._log_state(
                            current_time, time_since_last, task, "FAILSAFE", 0, 1, 0, time_since_last*1000,
                        ...
                        if current_time - last_warning_time > 1.0:
                            self.logger.warning(
                                f"\n[WARNING] TARGET {time_since_last:.1f} HAS BEEN MISSING FOR SECONDS! FAILSAFE ACTIVE: THE AIRCRAFT IS HELD AT STABLE TARGETS!!!")
