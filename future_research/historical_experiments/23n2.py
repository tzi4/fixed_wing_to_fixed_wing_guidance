...
class RedisListener(threading.Thread):
    ...
    #Enable only when the detector uses a 640x640 letterbox and publishes
    #boxes without converting them to original-image coordinates. Leave
    #False if the tracker already converts them. As a quick check, x or y
    #values above 640 show that the box is already in the original space.
    DELETTERBOX_ENABLED = True   # ← If Tracker already fixes it, set it to False
    ORIG_W = 1280                # Camera original width
    ORIG_H = 720                 # camera original height
    NET_W  = 640                 # Detector input width
    NET_H  = 640                 # Detector input height
    ...
    def __init__(self, data_queue):
        ...
        if self.DELETTERBOX_ENABLED:
            print(f"De-letterbox enabled: gain={self.lb_gain}, pad_x={self.lb_pad_x}, pad_y={self.lb_pad_y}")
                ...
                if message and message['type'] == 'message':
                    ...
                    try:
                        ...
                        if len(bbox_data) >= 4:
                            ...
                            # Letterbox correction (640x640 → 1280x720)
                            if self.DELETTERBOX_ENABLED:
...
class AutopilotController:
    def __init__(self, connection_str='udp:127.0.0.1:14552'):
        ...
        self.fy = self.fx  # frame pixel assumption (can be adjusted separately after calibration)

        print(f"Camera settings: fx={self.fx:.1f}, fy={self.fy:.1f}, HFOV={math.degrees(self.camera_hfov_rad):.1f}°")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # --- ERROR CALCULATIONS --- Raw pixel errors go directly into PID
                ...
                # --- SENDING CONTROL TO THE AIRCRAFT --- Aircraft attitude (from MAVLink)
                ...
                if task == "Visual" and self.mavlink.current_mode == "GUIDED":
                    ...
                    print(f"[{task}] AUTONOMOUS TRACKING: CmdRoll={final_roll_deg:.1f}° CmdPitch={final_pitch_deg:.1f}° | "
                          f"AC: R={ac_roll_deg:.1f}° P={ac_pitch_deg:.1f}° Y={ac_yaw_deg:.1f}° | "
                          f"Raw: px=({obj_x:.1f},{obj_y:.1f}) angle=({error_x_deg:.2f}°,{error_y_deg:.2f}°) | "
                          f"BBox: {bbox_w:.0f}x{bbox_h:.0f}")
                else:
                    print(f"[{task}] DEBUG: CmdRoll={final_roll_deg:.1f}° CmdPitch={final_pitch_deg:.1f}° | "
                          f"AC: R={ac_roll_deg:.1f}° P={ac_pitch_deg:.1f}° Y={ac_yaw_deg:.1f}° | "
                          f"Raw: px=({obj_x:.1f},{obj_y:.1f}) angle=({error_x_deg:.2f}°,{error_y_deg:.2f}°) | "
                          f"BBox: {bbox_w:.0f}x{bbox_h:.0f}")
