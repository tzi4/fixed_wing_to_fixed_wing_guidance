...
# ArduPilot stores only these commands as a Location, as defined by
#AP_Mission::stored_in_location in libraries/AP_Mission/AP_Mission.cpp.
#For other commands, such as DO_CHANGE_SPEED 178, readback returns
#frame = 0 and x = y = z = 0 because mission_cmd_to_mavlink_int fills
#position fields only when stored_in_location() is true. Validate
#param1 through param4 for those items instead of coordinates and frame.
LOCATION_COMMANDS = {
...
def read_plan(path):
    ...
    if len(home) != 3:
        raise ValueError('plannedHomePosition missing_fields')
    items = [{
        'seq': 0,
...
def upload_once(master, items):
    ...
    while time.monotonic() < deadline:
        ...
        # Always send MISSION_ITEM_INT, including in response to MISSION_REQUEST.
        #ArduPilot stores latitude and longitude as int32 values scaled by 1e7.
        #MISSION_ITEM uses float32 and can introduce errors around 3e-6 degrees
        #at distant waypoints, failing the strict abs_tol = 2e-6 readback check.
        #ArduPilot explicitly requests MISSION_ITEM_INT.
...
def download(master):
    ...
    for seq in range(int(count_msg.count)):
        ...
        if msg is None:
            raise TimeoutError(f"Could not read back task item: {seq}")
        result.append(msg)
...
def verify(expected, actual):
    ...
    for want, got in zip(expected, actual):
        ...
        # Elements that do not store a position (e.g. DO_CHANGE_SPEED): frame/coordinates always returns 0 on readback, param1..param4 is validated instead.
        ...
        if got_frame != want['frame']:
            raise RuntimeError(f"item {want['seq']} ID is different")
        # Can write back the bank 0 home coordinate ArduPlane with the active home value EKF; task items 1..N are verified one-to-one.
        if want['seq'] == 0:
            continue
        params = want['params']
...
def main():
    ...
    parser.add_argument('--plan', type=Path, default=root / 'missions' / 'straight.plan')
    ...
    for port, expected_sysid in args.ports:
        ...
        try:
            print(f"[{port}] SysID {expected_sysid} heartbeat expected...", flush=True)
            heartbeat = None
        ...
        time.sleep(0.5)
    if failures:
        raise SystemExit('Task installation failed: ' + '; '.join(failures))
