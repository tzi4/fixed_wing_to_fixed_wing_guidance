...
class BboxMonitor:
    """tracker_bbox Redis counts the messages coming to the pubsub channel in a specific window."""
...
def separation_3d(a, b):
    """Instant 3D distance of two vehicles (m); None unless both are in the air (rel_alt>10)."""
...
def connect(port, sysid, timeout):
    ...
    if heartbeat is None or received_sysid != sysid:
        master.close()
        raise RuntimeError(f"port {port}: SysID {sysid} expected, {received_sysid} received")
...
def main():
    ...
    parser.add_argument('--ready-file', type=Path, help='Create MAVLink connections when ready')
