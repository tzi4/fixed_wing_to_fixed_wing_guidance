...
# oraya bağlanan kod heartbeat beklerken sonsuza kadar takılır.
...
class AutopilotController:
    def __init__(self, connection_str=DEFAULT_CONNECTION, target_sysid=DEFAULT_SYSID):
        ...
        # 2026-07-26 SITL/Gazebo ölçümü: /webcam/camera_info
        ...
        # ESKİ DEĞERLER (gerçek 1920x1080 kameradan kalmış, sim ile uyumsuz):
        #   width=1920 height=1080 hfov=0.42 center=(1091,633) fx=4515 fy=4510
        # Bu değerlerle bbox merkezi (640,360) iken kod hedefi 5.7° sağda ve
        # 3.5° aşağıda sanıyor; Kp_heading=3.0 ile ~30° yanlış dönüş komutu
        # üretip hedefi kadrajdan atıyordu (ölçüldü: GUIDED'da 1.2 s TRACKING).
        # PID/kazanç/geometri matematiğine dokunulmadı; yalnız kamera
        # kalibrasyonu gerçek sisteme göre güncellendi.
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                rakip_irtifa = self.redis.get_rakip_irtifa()
                irtifa_farki = 0.0
                if rakip_irtifa is not None:
                    # Görsel PID irtifası ile gerçek rakip irtifası arasındaki fark
                    irtifa_farki = target_alt - rakip_irtifa
                # ----------------------------------------------------------
                ...
                if current_time - self.last_cmd_send_time >= 0.1:
                    if gorev == "Goruntulu" :
                        ...
                        if rakip_irtifa is not None:
                            logging.info(f"[{gorev}] OTONOM: Yön: {target_heading:.1f}° | Görsel İrtifa: {target_alt:.1f}m | ham y: {bbox_y:.1f} stab y: {stab_y:.1f} Rakip İrtifa: {rakip_irtifa:.1f}m | Fark: {irtifa_farki:.1f}m")
                        else:
                            logging.info(f"[{gorev}] OTONOM: Yön: {target_heading:.1f}° | İrtifa: {target_alt:.1f}m | ham y: {bbox_y:.1f} stab y: {stab_y:.1f} (Rakip verisi yok)")

                        # print(f"[{gorev}] OTONOM: HedefYön: {target_heading:.1f}° | Rate: {heading_rate:.1f}°/s | İrtifa: {target_alt:.1f}m")
                    else:
                        print(f"[{gorev}] BEKLEME - Hdf Yön: {target_heading:.1f}° | Görsel İrtifa: {target_alt:.1f}m | ham y: {bbox_y:.1f} stab y: {stab_y:.1f} Rakip İrtifa: {rakip_irtifa:.1f}m | Fark: {irtifa_farki:.1f}m" if rakip_irtifa is not None else f"[{gorev}] BEKLEME - Hdf Yön: {target_heading:.1f}° | İrtifa: {target_alt:.1f}m | Rakip verisi yok")
...
yeni ucak 16 mm 1920x1080 mindvision ile 25 kilit atildi
