...
#Port 14562 has no output and would wait indefinitely for a heartbeat.
...
class AutopilotController:
    def __init__(self, connection_str=DEFAULT_CONNECTION, target_sysid=DEFAULT_SYSID):
        ...
        self.max_alt_change_m = 2
        self.min_alt_change_m = -5    
        ...
        # CAMERA SETTINGS: VERIFY AGAINST THE ACTUAL INSTALLATION
        #Measured through /webcam/camera_info in SITL/Gazebo on 2026-07-26:
        #  width = 1280, height = 720
        #  K = [4711.90568789373, 0, 640.5, 0, 4711.90568789373, 360.5, 0, 0, 1]
        #  models/bumblebee/model.sdf: <horizontal_fov>0.27</horizontal_fov>
        #
        #The previous calibration described a real 1920x1080 camera:
        #  hfov = 0.42, center = (1091,633), fx = 4515, fy = 4510.
        #Using those values in this simulation interpreted a centered box at
        #(640,360) as 5.7 degrees right and 3.5 degrees down. With Kp_heading = 3.0,
        #this generated an erroneous turn of about 30 degrees and lost the target.
        #Measured GUIDED tracking lasted only 1.2 s.
        #The camera calibration was updated to match the simulation, with the
        #PID gains and geometric-control equations retained.
        ...
        self.center_x = 640.5
        self.center_y = 360.5
        self.fx = 4711.90568789373
        self.fy = 4711.90568789373
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                cmd_alt_m = self.clamp(cmd_alt_m, self.min_alt_change_m, self.max_alt_change_m)
                ...
                target_altitude = self.redis.get_target_altitude()
                altitude_difference = 0.0
                if target_altitude is not None:
                    # Difference between visual PID altitude and actual competitor altitude
                    altitude_difference = target_alt - target_altitude
                # ----------------------------------------------------------
                ...
                if current_time - self.last_cmd_send_time >= 0.1:
                    if task == "Visual" :
                        ...
                        if target_altitude is not None:
                            logging.info(f"[{task}] AUTONOMOUS: Direction: {target_heading:.1f}° | Visual altitude: {target_alt:.1f}m | raw y: {bbox_y:.1f} stab y: {stab_y:.1f} Competitor altitude: {target_altitude:.1f}m | Difference: {altitude_difference:.1f}m")
                        else:
                            logging.info(f"[{task}] AUTONOMOUS: Direction: {target_heading:.1f}° | altitude: {target_alt:.1f}m | raw y: {bbox_y:.1f} stab y: {stab_y:.1f} (No competitive data)")

                        # print(f"[{task}] AUTONOMOUS: TargetDirection: {target_heading:.1f}° | Rate: {heading_rate:.1f}°/s | altitude: {target_alt:.1f}m")
                    else:
                        print(f"[{task}] STANDBY - Hdf Direction: {target_heading:.1f}° | Visual altitude: {target_alt:.1f}m | raw y: {bbox_y:.1f} stab y: {stab_y:.1f} Competitor altitude: {target_altitude:.1f}m | Difference: {altitude_difference:.1f}m" if target_altitude is not None else f"[{task}] STANDBY - Hdf Direction: {target_heading:.1f}° | altitude: {target_alt:.1f}m | No rival exchange")
...
25 locks were fired with the new aircraft 16 mm 1920x1080 mindvision
