#!/usr/bin/env bash
# It checks the installation without starting the process. Default headless; --gui also controls QGC/gzclient.
set -u
...
check_command() {
  command -v "$1" >/dev/null 2>&1 && ok "command: $1" || bad "command: $1"
}

check_path() {
  [[ -e "$1" ]] && ok "file: $1" || bad "file: $1"
}

echo "Erenimbus/Bumblebee installation control"
...
for required_path in \
  ...
  "$ROOT_DIR/models/emir_aircraft_temp/model.sdf" \
  "$ROOT_DIR/models/purple_target/model.sdf" \
...
PY
then
  ok "Python/ROS modules"
else
  bad "Python/ROS modules"
fi

if [[ "$failed" -ne 0 ]]; then
  echo "RESULT: installation is incomplete. Check the INSTALL.md steps and ways above." >&2
  exit 1
fi
echo "RESULT: the installation is ready for initialization."
