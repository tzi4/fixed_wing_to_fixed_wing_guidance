...
# Simulation camera (EXACTLY REAL HARDWARE since 2026-07-26): models/bumblebee/model.sdf -> 1920x1080, horizontal_fov=0.42 rad -> fx = fy = 960/tan(0.21) = 4504.03 px, cx=960, cy=540.  Vertical FOV = 2*atan(540/4504.03) = 0.2386 rad (±6.837°).
CAMERA_HFOV_RAD = 0.42
CAMERA_FX = (RESOLUTION_W / 2.0) / math.tan(CAMERA_HFOV_RAD / 2.0)
CAMERA_FY = CAMERA_FX            # frame pixel assumption

# THE REAL CAMERA PROFILE is NOT a separate profile anymore: the above values ​​are also valid in flight as the sim sensor is pulled to real spec (1920x1080, 0.42 rad).  The camera mount tilt has also been implemented for parity on the model.sdf (2.678° nose up), so the virtual gimbal math works with the same numbers on both sides.  Old (1280x720) sim profile: HFOV 0.27, fx 4711.9057 — no longer used.
...
class TestCommander(threading.Thread):
    """
    Send commands periodically in a thread independent of image processing.
...
into ArduPlane's required acceleration using a = V * omega.
    ...
    def _send_airspeed(self, airspeed_ms):
        """MAV_CMD_DO_CHANGE_SPEED (178). Shipped in GUIDED mode only."""
...
Heading uses proportional control plus a rate PID and angular error.
Altitude uses PD control plus a rate PID and angular error.
Airspeed uses coverage-based PID control with a slew-rate limit.
These components follow initial_camp_emir and tzi_final.
    """
    ...
    def __init__(self):
        ...
        self.min_speed = 14.0         # above AIRSPEED_MIN
        self.max_speed = 22.0
        self.base_speed = 17.0        # Neutral speed (cruise)
        self.target_coverage_pct = 6.0    # Following distance to be maintained (%)
        self.speed_integral_band = 5.0    # Accumulate i only within this band
        self.speed_slew_rate = 1.0        # Max speed change (m/s/s) (from tzi_final)
    ...
    def guide_aircraft(self, bbox, current_time):
        ...
        # Integral (band-limited, from initial_camp_emir)
