...
class TestCommander(threading.Thread):
    """
      ...
      - Airspeed: controlled by parameter TRIM_THROTTLE
    """
    ...
    def _send_throttle(self, throttle_pct):
        """Update parameter TRIM_THROTTLE with param_set.         Only allow throttle change when in GUIDED mode.         TRIM_THROTTLE: TECS cruise throttle (50-127).         TECS automatically updates the target speed when this parameter is changed at runtime.
        """
...
class tziGuidance(System):
    def __init__(self):
        ...
        self.throttle_integral_min = -10.0  # Asymmetric integral lower limit
        self.throttle_integral_max = 55.0   # Asymmetric integral upper limit
        self.throttle_base = 50.0  # Initial value of TRIM_THROTTLE (to be added by PID)
    ...
    def guide_aircraft(self, bbox, current_time): #bbox: x1,y1,w,h
        ...
        if self.K_inv is not None:
            ...
            self.logger.debug(f"Virtual Gimbal: u_virt={u_virt:.2f}, v_virt={v_virt:.2f} | Raw Error: ex={object_x    :.2f}, ey={object_y:.2f} | BBox: {bbox} ({bbox_w}x{bbox_h}, sqrt={bbox_sqrt_area:.1f}) | RPY(rad): {roll_rad:.3f},{pitch_rad:.3f},{yaw_rad:.3f} | Mode: {current_mode}")
