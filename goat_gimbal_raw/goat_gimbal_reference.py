...
class MavlinkManager(threading.Thread):
    ...
    def send_airspeed_target(self, airspeed_ms):
        with self.lock:
            try:
                self.master.mav.command_long_send(
                    self.master.target_system,
...
class AutopilotController:
    def __init__(self, connection_str='udp:127.0.0.1:14552'):
        ...
        self.Kp_alt = 0.7       # 1 degree error = 1.5 meters altitude change
        self.Ki_alt = 0.0
        self.Kd_alt = 0.12
        ...
        self.target_airspeed = 20.0  # Default speed for sequential tuning (m/s)
            ...
            while True:
                        ...
                        if td <= 5.0:
                            ...
                            if task == "Visual" and self.mavlink.current_mode == "GUIDED":
                                if current_time - self.last_cmd_send_time >= 0.2: # 5Hz renewal
                                    ...
                                    rate = self.last_heading_rate
                                    alt = self.last_target_alt
                                    spd = self.target_airspeed
                                    
                                    # self.mavlink.send_heading_target(hdg, rate)
                                    # self.mavlink.send_altitude_target(alt)
                                    self.mavlink.send_airspeed_target(spd)
                ...
                if current_time - self.last_cmd_send_time >= 0.1:
                    if task == "Visual" and self.mavlink.current_mode == "GUIDED":
                        ...
                        command_sent = 1
                        print(f"[{task}] AUTONOMOUS: TargetDirection: {target_heading:.1f}° | Rate: {heading_rate:.1f}°/s | altitude: {target_alt:.1f}m | Speed: {spd:.1f}m/s")
                    else:
...
if __name__ == '__main__':
    guidance = AutopilotController(connection_str='udp:127.0.0.1:14553')
    guidance.run()
