...
# ─── CONSTANTS (config values ​​used in tzi2.py) ───
RESOLUTION_W = 640
RESOLUTION_H = 480
...
Airspeed: MAV_CMD_GUIDED_CHANGE_SPEED (43000), following Sam Hyams's method.
    """
    ...
    def _send_airspeed(self, airspeed_ms):
        """MAV_CMD_GUIDED_CHANGE_SPEED (43000), following Sam Hyams's blog method.

Send airspeed commands only in GUIDED mode.
param1: SPEED_TYPE, 0 for airspeed or 1 for ground speed.
param2: target speed in m/s.
param3: acceleration in m/s squared, with zero applying the change immediately.
        """
...
class tziGuidance(System):
    def __init__(self):
        ...
        # AIRSPEED PID Variables Proportional scaling: throttle range [50,127]=77 → airspeed range [10,22.8]=12.8 scale = 12.8/77 = 0.1662, original proportions are preserved (kp:ki:kd = 1:0.096:0.128)
        ...
        self.airspeed_integral_min = -1.66  # Asymmetric integral lower limit (-10 * 0.1662)
        self.airspeed_integral_max = 9.14   # Asymmetric integral upper limit (55 * 0.1662)
        self.airspeed_base = 15.0  # Initial airspeed value (m/s, to be added PID)
    ...
    def guide_aircraft(self, bbox, current_time): #bbox: x1,y1,w,h
        ...
        if self.K_inv is not None:
            ...
            # --- RAW and VIRTUAL GIMBAL ERROR CALCULATIONS --- Raw error (raw pixel center difference)
            ex_raw = object_x - self.center_x
            ey_raw = object_y - self.center_y

            # Virtual gimbal error (stabilized pixel center difference)
            ex_vg = u_virt - self.center_x
            ey_vg = v_virt - self.center_y

            # Detailed head to terminal and log
            info_line = (
            ...
            # Error calculation: target - current (attempting to reach target size)
            ...
            # Limit output (Min airspeed_min, Max airspeed_max)
            airspeed_out = max(self.airspeed_min, min(self.airspeed_max, raw_airspeed))
            self.test_airspeed_ms = airspeed_out

            # --- HEADING PD Controller --- If target job on the right (u_virt > 320), increase heading (turn right).
            ...
            # --- ALTITUDE PD controller --- If target is below center (v_virt > 240), decrease altitude.
            ...
            # Also log measured airspeed (for command vs actual comparison)
    ...
    def _vision_processor(self):
        """Thread 2: Reads the latest bbox at a fixed frequency (30 Hz) and calls guide_aircraft(). It does not reprocess the same bbox."""
