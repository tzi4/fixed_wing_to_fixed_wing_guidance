...
class AutopilotController:
    def __init__(self, connection_str='udp:127.0.0.1:14552'):
        ...
        # └──────────┴────────────┴────────────┴────────────┘
        # Read the lens focal-length marking and enter the corresponding HFOV.
        # If uncertain, start with a moderate focal length of about 16 mm.
        
        self.camera_width = 1280  
        self.camera_height = 720 
        self.camera_hfov_rad = 0.349  # ← Select from above table according to lens mm (currently 16mm)
            ...
            while True:
                ...
                if task == "Visual" and self.mavlink.current_mode == "GUIDED":
                    ...
                    print(f"[{task}] AUTONOMOUS TRACKING: Roll={final_roll_deg:.1f}° Pitch={final_pitch_deg:.1f}° | "
                          f"Gimbal Before: px=({obj_x:.1f},{obj_y:.1f}) angle=({raw_error_x_deg:.2f}°,{raw_error_y_deg:.2f}°) | "
                          f"Gimbal After: px=({stab_x:.1f},{stab_y:.1f}) angle=({stab_error_x_deg:.2f}°,{stab_error_y_deg:.2f}°) | "
                          f"BBox: {bbox_w:.0f}x{bbox_h:.0f}")
                else:
                    print(f"[{task}] DEBUG: Roll={final_roll_deg:.1f}° Pitch={final_pitch_deg:.1f}° | "
                          f"Gimbal Before: px=({obj_x:.1f},{obj_y:.1f}) angle=({raw_error_x_deg:.2f}°,{raw_error_y_deg:.2f}°) | "
                          f"Gimbal After: px=({stab_x:.1f},{stab_y:.1f}) angle=({stab_error_x_deg:.2f}°,{stab_error_y_deg:.2f}°) | "
                          f"BBox: {bbox_w:.0f}x{bbox_h:.0f}")
