...
class GOATGuidance(System):
    ...
    def init_control_PID(self):
        ...
        self.pitch_pid = PID(1, 0.0, 0, setpoint=-0.1, # setpoint means desired target value, positive setpoint will mean that the enemy will be placed in the under half of the screen -0.24 daha yukarıda sabitlemeye denk gelir.
                             output_limits=(-1.0, 1.0))
        ...
        for message in self.p.listen():
            ...
            if message['type'] == 'message':
                ...
                if not isinstance(data, (list, tuple)):
                    # Ignore dictionary messages (likely from detection_son.py)
                    continue
